...
local function open(name,frames)
  local s=app.open(root..'tools/haiwang_trident_'..name..'.aseprite')
...
local body,gun=open('body_atlas',1),open('gun_atlas',1)
assert(body.width==1024 and body.height==1024 and gun.width==1024 and gun.height==1024)
local held,run,jump,crouch=open('held-v2',1),open('run-v3',8),open('jump-v2',18),open('crouch-v2',9)
...
jump:saveAs(root..'tools/haiwang_trident_jump-v2.aseprite')
local bodyCells,gunCells,checks={},{},{}
...
local actions=open('actions',36)
assert(#actions.layers==4 and not actions.layers[1].isVisible,'Attack body reference must stay hidden')
for f=1,36 do
  local group=math.floor((f-1)/9)
  local cell=math.floor(group/2)*32+(group%2)*16+(f-1)%9
  put(gunCells,cell,flat(actions,f))
end
...
body:saveAs(root..'tools/haiwang_trident_body_atlas.aseprite')
gun:saveAs(root..'tools/haiwang_trident_gun_atlas.aseprite')
...
local projectile=open('projectile',2)
...
for _,range in ipairs(ranges) do
  ...
  if range[1]=='zipline-v2' and app.params['zipline'] then
    ...
    s:saveAs(root..'tools/haiwang_trident_zipline-v2.aseprite');fixed:close()
  end
  for f=1,range[2] do
...
traversal:saveAs(root..'tools/haiwang_trident_traversal-v2.aseprite')
for _,s in ipairs({body,gun,held,run,jump,crouch,actions,projectile,traversal}) do s:close() end
print('Verified 44 movement body cells, 36 paired weapons, 36 attack poses, 86 traversal source frames and both landing transitions.')
