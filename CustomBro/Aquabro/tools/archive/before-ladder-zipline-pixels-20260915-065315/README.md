...
| [haiwang_trident_wall-v2.aseprite](haiwang_trident_wall-v2.aseprite) | 贴墙独立动画，空手，10 帧；对应总稿 1–10 帧 |
| [haiwang_trident_hanging-v2.aseprite](haiwang_trident_hanging-v2.aseprite) | 悬挂独立动画，空手，18 帧；对应总稿 11–28 帧 |
| [haiwang_trident_climbing-v2.aseprite](haiwang_trident_climbing-v2.aseprite) | 攀爬独立动画，空手，20 帧；对应总稿 29–48 帧 |
...
修改分帧动作后，需要同步到对应图集并导出 PNG；图集不会自动跟随源文件更新。当前持戟站姿、跑步、跳跃、落地、蹲持、蹲走，以及贴墙、悬挂、攀爬、爬梯和滑索 v2 均已同步到游戏图集及代码，构建部署结果见 [反馈记录](haiwang_trident_反馈记录.md)。`RenderTrident()` 按实际显示的身体格和攻击姿态选择武器帧，`TridentMovementAnimation` 记录配对格号、滑索重定向和落地时序。
