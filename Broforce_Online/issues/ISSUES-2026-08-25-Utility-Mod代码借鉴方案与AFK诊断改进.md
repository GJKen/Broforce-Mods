...
## 必须改进：UMM 折叠分组与可勾选日志审查

这不是本轮已经实现的功能，而是用户确认的日后必须改进项。Utility Mod 的 UMM 设置页通过多个布尔状态保存分组展开状态，再用按钮切换并只绘制已经展开的区域。当前项目仍把 Workshop、诊断和 FRP Direct 控件平铺在一个页面；随着选项增加，查找和审查都会越来越困难。

### 设置界面折叠分组

当前 UMM 设置页必须改成以下折叠分组：

- `Workshop 联机`：地图注入、AFK 开关、Workshop ID、战役名和场景名。
- `诊断日志`：会话 ID、标签、日志类别选择和诊断预设。
- `FRP Direct`：传输开关、Host/Client、端点、密码、状态和应用按钮。

建议默认展开 `Workshop 联机`，默认收起 `诊断日志` 和 `FRP Direct`；用户展开或收起后的状态应写入设置，下次打开仍保持。折叠只改变界面显示，不能改变功能是否启用。

### 日志类别必须可勾选

诊断日志不能继续只有全开或全关，必须允许用户按问题类型勾选。初步分类如下：
...
分类开关只能控制诊断信息是否输出，不能关闭补丁、改变联机流程或影响游戏规则。可以提供 `基础联机`、`加入/重入问题`、`AFK/失败问题`、`Workshop 加载问题` 和 `完整诊断` 等快捷预设，但仍需保留逐项勾选，方便针对具体问题微调。
...
| UMM 折叠分组与可勾选日志审查 | 未实现，必须实施 | 折叠保存界面状态；日志按 Bug 类型选择，并保留不可关闭的核心证据 |
...
- 必须实现 UMM 功能折叠分组和日志类别勾选；AI 复查 Bug 时应明确指定所需类别，避免采集过多无用信息。
