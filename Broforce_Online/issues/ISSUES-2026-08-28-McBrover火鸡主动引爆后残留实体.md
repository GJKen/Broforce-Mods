...
- 部署本次窄范围同步补丁并完成实机复测后，用户再次确认仍会出现二次遗留火鸡；与修复前相比，用户观察到复现概率降低，但本轮没有记录完整的尝试总数，不能给出可计算的百分比。
...
已有两个用户确认的异常样本。修复后观察到概率降低，但尚未形成受控的分子/分母统计，不能把“降低”写成确定的复现率。后续复测应补充多个正常与异常样本，并记录每次 NID、拥有权、Death 次数、爆炸次数、最终销毁状态和地形变化。
...
- 本节只证明实现、构建和部署完成；双方重启后的联机实测已确认问题仍可复现，修复效果仅表现为用户观察到的概率降低。
