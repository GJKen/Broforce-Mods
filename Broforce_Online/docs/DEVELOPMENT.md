...
手工构建时必须使用 `<local-installation-path>`，并显式引用 .NET 2.0 的 `mscorlib`/`System` 和 .NET 3.5 的 `System.Core`。不得使用 `v4.0.30319\csc.exe` 直接构建；该方式会在 DLL 中混入 .NET 4.0 引用，而 Broforce 的 Unity/Mono 运行时只兼容 .NET 2.0/3.5。
