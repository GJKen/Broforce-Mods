...
- 当前构建修复了加入方退出房间后重新加入时缺少房主角色实例的问题。本轮 40 秒双端 MCP 监控确认，房主 P1 的 buffered `PlayerPrefab` 和角色在加入方仍处于 `MainMenu` 时提前实例化，随后随 Workshop 场景加载被销毁；上一构建重发的原生 `HeroController.AddPlayer` 对远程 PID 只重置 Ping，并不会创建远程 `Player`。当前构建改为在房主处理最终场景中的 `RequestJoinGame` 后，向该加入方重放房主当前的 buffered 网络实例。`buildHash=f4be08d8d30129049f3acd003c93116e077edaf5f3be3cda8a9ce1faac8701a5` 已通过 FRP Direct 双端退出房间和重新加入实测，加入方最终场景持续保留房主 P1。
...
当前仍需继续验证：新增诊断在真实双端会话中的日志内容，尤其是 AFK 触发后房主是否仍把已移除槽位计入存活人数；道具修复在官方 Steam 大厅 Workshop 会话中的独立复测，以及异常断网、长期多轮退出/重入、多地图兼容和高延迟环境。部分 Workshop 地图还可能在 `GeneratePole.Awake`、`BroBase` 或特效销毁流程抛出自身运行错误。详细实现、测试边界和历史证据见 [开发与测试文档](docs/DEVELOPMENT.md) 与 [问题记录索引](issues/README.md)。
...
晚加入、掉线槽位恢复和控制器修复所需的诊断日志关键字统一记录在 [开发与测试文档](docs/DEVELOPMENT.md#晚加入支持) 中。当前分发构建的远程玩家槽位恢复已通过本轮 FRP Direct 退出房间和重新加入实测；更多地图、高延迟和长期多轮重入仍需继续验收。
...
当前实验层只支持房主和一台远端机器之间的连接；每台机器仍可使用 Broforce 原生本地玩家槽位。重复加入请求会复用现有身份，远端离开时房主大厅继续保留。第一版尚未实现 FRP 主机迁移，房主离开会结束房间；当前地图上的加入方正常退出房间后重新加入已通过实测，异常断网重连、长期多轮测试、不同 Workshop 地图和高延迟环境仍需验收。
