...
当前构建保留普通网络 Mook 的死亡与尸体终态收敛：拥有端按 NID 广播死亡冲量和最终停稳位置，远端用非空 `DamageObject` 补全死亡链。它不提供活动 AI 的持续状态同步、敌方弹体网络生成、钱币权威生成或金色奖励的坐标键同步。Workshop 道具范围仅包括标准弹药箱内容确定、远端角色不扫描本地道具、重复收集抑制和弹药满时的退避。

`DemolitionBro` 主动引爆继续保留拥有端立即执行原版 `Death()`、向其它端发送 NID 和位置的窄范围同步。`McBrover` 火鸡使用相同范围的同步与幂等诊断；用户已实机确认残留火鸡仍可复现，但发生概率显著降低，尚无受控样本总数可量化，详见 [独立 issue](issues/ISSUES-2026-08-28-McBrover火鸡主动引爆后残留实体.md)。普通 `Grenade` 地形伤害、动态世界、钱币和金色奖励的历史实验不在当前构建中。
...
仍需继续覆盖：普通 Mook 死亡终态和尸体落点的重启后双端实测、McBrover 火鸡主动引爆残留、AFK 诊断在真实双端会话中的内容、官方 Steam 大厅下的道具修复、多地图兼容、高延迟和长期多轮退出/重入。FRP 的房主加两台加入方（三机）基础联机及 `1` 人房满员提示已通过用户实测；四机、`2` 至 `4` 人容量边界、动态降额重入和主机迁移仍未验证。部分 Workshop 地图可能有自身的 `GeneratePole.Awake`、`BroBase` 或特效销毁异常。详细实现和证据见 [开发与测试文档](docs/DEVELOPMENT.md)、[问题记录索引](issues/README.md)。
