...
聊天输入的关键修复已通过窄范围真实键盘回归：中文 IME、数字/符号、原生编辑、500 UTF-16 字符上限、viewport、视觉行 Up/Down、字数显示和发送清空。`Enter -> Esc -> Esc -> Enter` 后无法再次呼出及重新打开后无法发送的问题也已修复。输入框激活时拦截当前键盘角色操作已通过真实键盘回归，移动、攻击、关闭后恢复及仅显示历史/提示时不拦截均已确认，详见[聊天输入专题](CHAT.md)和[对应问题记录](../issues/ISSUES-2026-09-07-联机聊天输入框激活时角色仍可操作.md)。完整输入矩阵和聊天历史的长期边界仍需专项验收。

后续验收只需覆盖：混合文本和换行、开头/中间/末尾编辑、IME 候选后移动、发送后重开、长消息历史可见性，以及多条消息超出边界时只隐藏旧消息。

相关专项记录：[中文输入法](../issues/ISSUES-2026-09-05-联机聊天中文输入法支持与关键改动.md)、[长消息 viewport](../issues/ISSUES-2026-09-05-联机聊天输入框长消息可视滚动效果不理想.md)、[视觉行上下移动](../issues/ISSUES-2026-09-06-联机聊天输入框上下方向键跨视觉行移动.md)、[字数显示](../issues/ISSUES-2026-09-06-联机聊天输入框右下角字数显示.md)、[历史消息显示](../issues/ISSUES-2026-09-06-联机聊天长消息发送后历史显示被隐藏.md)和[输入框激活时角色仍可操作](../issues/ISSUES-2026-09-07-联机聊天输入框激活时角色仍可操作.md)。
