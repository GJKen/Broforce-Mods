...
- Client 离开或断线只清理该机器的 PID；Host 离开会结束房间，不支持主机迁移。用户对照测试确认 FRP 房主退出后加入方直接返回主菜单，本 issue 的 Steam 黑屏不适用于 FRP。RTT 为各机器至 Host 的往返时间。
...
FRP 房主退出后的加入方返回路径已通过对照测试确认直接返回主菜单；FRP 仍不支持 Host migration，不应把该正常结束房间路径误记为主机迁移或 Steam 黑屏问题。
