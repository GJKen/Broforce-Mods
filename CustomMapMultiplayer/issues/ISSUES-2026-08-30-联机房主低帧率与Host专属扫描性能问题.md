...
| 用户实测反馈 | `5582c...` 双端短测未复现此前持续 24–29 FPS；Host 仍比 Client 略重 |
| 正式性能验收 | 本轮不再安排 A/B；保留短测观察数据，不作性能归因 |
| 加入方箱子坍塌特效异常 | 已独立记录，不纳入本轮性能判定 |
| Test_Evan2 原生对象/投掷物空引用 | 已独立记录，不纳入本轮性能判定 |
| 本地订阅地图选择后个别地图返回主菜单 | 已记录，暂不纳入本轮性能修复 |

当前方案继续保留 UMM/Harmony Mod 架构，没有替换 `Assembly-CSharp.dll`。用户已明确本轮不需要正式 A/B 验收；现有帧时间和酸池统计仅作运行观察，不据此宣称问题已完全解决或进行跨设备性能归因。

## 范围与结论

本 issue 关注 Workshop 联机中 Host 帧时间明显高于 Client 的问题，重点检查 Mod 增加的 Host 专属扫描、实体终态同步和 Harmony Trace 开销。

当前最合理的结论是：Workshop 酸液扫描仍是已确认的 Mod 热点，实体终态遍历和 Harmony Trace 的单次开销已较低；原生 Host 的 AI、物理和网络权威负载未作进一步对照。第一方案已经降低了前三条 Mod 路径的开销，`5582c...` 双端短测未复现此前持续低帧率；本轮不再追加正式 A/B 验收。
...
阶段 1 已完成编译；启动和最小 Workshop 联机回归已完成，后续不再安排本轮性能 A/B；不在这一阶段加入 Trace 结构重写或新的网络行为。
...
用户于 2026-08-31 明确本轮不需要 A/B 验收。以下对照矩阵和门槛保留为历史方案，不是当前待办；当前只保留已有运行观察和功能问题的独立跟踪。
...
- 日志时间：UTC `2026-08-30 17:12:58` 至 `17:18:09`；地图：`Test_Evan2`；Workshop ID：`3715087178`。
...
- 排除 `p99=250ms` 的地图重载窗口，并按摘要 payload 的 `role` 排除 Client 在退出前晋升为 Host 后的 222 帧窗口后，Host 为 `23,105` 帧，加权平均 `11.284ms`，p50/p95/p99 为 `11/17/21ms`；Client 为 `29,805` 帧，加权平均 `9.149ms`，p50/p95/p99 为 `9/14/19ms`。此前 `30,027` 帧、`9.148ms` 的 Client 数字包含该迁移后 Host 摘要，现已修正。
- 本轮没有复现此前持续性的 Host 低帧率；两端图形设置未记录，且没有同条件优化前基线或反向 Host，因此按用户决定仅记为“短测观察到改善”，不再追求正式 A/B 结论。
- Host 酸池刷新 `217` 次，累计 `2264.130ms`，单次最高 `12.984ms`；本轮 `acidHook=0`，没有实际接触酸液。酸池扫描仍是后续最明确的 Mod 性能热点，但不能解释地图重载期间的 `200–250ms` 停顿。
- Client 于 `17:18:07.906Z` 记录 `Online session role changed from client to host`；`17:18:09.043Z` 的最后一条摘要因此带 `role=host`，不计入 Client 稳定窗口。该 Host migration 发生在 `LeaveMatch` 前的退出阶段，不能作为反向 Host 性能 A/B。
- 本轮唯一的 Host Unity 错误是原生 `StepOn` 参数异常；Client 有 5 条 `ENTITY_FINAL` 等待超时。两者均不经过本轮性能路径，应从性能样本中单独标记。

## 当前结论

第一方案已覆盖当前确认的三条 Mod 性能路径，阶段 1 低风险细化优化和高频日志/反射减负候选均已落地；`5582c...` 双端短测说明严重低帧率没有再次出现，当前仍标记为“观察到改善”。按用户决定，本轮不再安排统一设置、交换 Host 或多轮 p50/p95/p99 对照，也不据此扩大酸液扫描或 Trace 修复范围。不要把原生 Host 负载、`StepOn`、实体终态超时、`DoodadCrate` 历史错误循环或“本地订阅地图选择后个别地图回菜单”混入本轮性能判定。
