...
| --- | --- | --- |
| `ISSUES-2026-08-30-加入方箱子坍塌特效持续重复.md` | Reproduced; root cause unconfirmed | The joining side repeatedly renders crate effects and reports many `DoodadCrate` errors about every five seconds. It strongly overlaps with the `BroforceBugFix` explosive-crate recursion issue, but cannot yet be merged with that diagnosis. |
| `ISSUES-2026-08-30-联机房主低帧率与Host专属扫描性能问题.md` | Improvement observed in long test; formal two-sided A/B pending | The latest `993e...` build was used in a long high-density combat test, and the user reported noticeably fewer Host frame drops. Acid-scan caching/rate limiting, entity final-state candidate iteration, final-state TTL, Mook qualification caching, and high-frequency log/reflection reductions are implemented; unified settings, reverse roles, and p50/p95/p99 comparisons remain. |
