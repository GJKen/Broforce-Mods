...
| --- | --- | --- |
| `ISSUES-2026-09-01-新增主动AFK按钮.md` | Feature implemented; development issues fixed; accepted in two-sided testing | Adding the “Enter AFK now” button exposed three stages: targeting the wrong role, automatic and repeated re-entry after manual AFK, and an explicitly returning player left at `lives=0` and `character=null`. The target now uses local ownership and input-controller matching; manual AFK waits for an explicit return and restores the character, while ordinary network-dropout recovery remains automatic. All related scenarios passed two-sided acceptance testing. |
