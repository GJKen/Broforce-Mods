...
- The current transport is official Steam Lobby/Steam P2P. Single-joining-player Host migration after a Steam Host leaves has been verified by the user; multi-player host migration, four-player play, capacity boundaries, and dynamic adjustment still require testing.
...
| --- | --- | --- |
| `ISSUES-2026-09-10-联机加入方可踢出房主.md` | Fixed and accepted in user testing, including a joining player without the Mod | `6d3761c9...` rejects unauthorized `KickRPC`, the associated `DropoutRPC`, and the native lobby kick notification on the Host; the user confirmed that an unmodded joining player cannot kick the Host and the Host side has no kick explosion. |
| `ISSUES-2026-09-08-Workshop地图选择与联机加载流程优化.md` | Workshop directory, title lookup, and map selection page implemented; native multiplayer loading flow simplification accepted by the user; latest runtime screen still needs a fresh MCP retest | The current page switches the UMM content area, uses a two-column grid, search/filter/favorite/recent-online-map controls, and manual Workshop ID input; the Starred tab can clear all starred maps at once. It preserves not-installed, incomplete-download, and unreadable-campaign states, P1-P4 confirmation, and the native loading animation. |
| `ISSUES-2026-09-05-Workshop房主退出后加入方返回黑屏.md` | Single-joining-player Steam Host migration after a Host leaves was corrected from the latest two-sided logs and verified by the user; multi-player migration still needs dedicated regression; FRP Direct comparison returns normally to MainMenu | Records the false Host-migration decision after a Steam Host leaves, the Workshop load loop and black screen, and the corrected exit handling; the Steam single-joining-player path now preserves official Host migration, while FRP Direct does not support Host migration and its joining player returns directly to MainMenu. |
