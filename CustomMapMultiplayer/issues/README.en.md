...
| --- | --- | --- |
| `ISSUES-2026-09-01-新增ESC菜单主动AFK按钮.md` | AFK feature and related issues consolidated; accepted in two-sided testing | Consolidates the manual AFK button, the single-player notice, wrong-role targeting, automatic and repeated re-entry after manual AFK, an explicitly returning player left at `lives=0` and `character=null`, and duplicate-button/action-shift issues after Esc-menu language changes. The target now uses local ownership and input-controller matching; manual AFK waits for an explicit return and restores the character, while ordinary network-dropout recovery remains automatic. |
